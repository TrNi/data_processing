...
# ── Load & parse ──────────────────────────────────────────────────────────────
col_names = ["folder_name", "col1", "col2", "col3", "col4", "col5", "PSNR", "SSIM", "LPIPS"]
raw = pd.read_csv("./Drbokeh/drbokeh_all_fl_28.csv", names=col_names, header=None, low_memory=False)

data_rows = raw[raw["folder_name"].str.startswith("drbokeh_K", na=False)].copy()
data_rows = data_rows[data_rows["PSNR"].notna()].copy()
...
df = (
    data_rows[["K", "fp", "PSNR", "SSIM", "LPIPS"]]
    .drop_duplicates(subset=["K", "fp"])
...
FIG_HEIGHT = TEXTWIDTH_IN * 0.35  # Scaled down to match reference script style
...
# ── ENLARGED SUBPLOT SPACING ──────────────────────────────────────────────────
# Shifted from 0.09 gap to a wider 0.12 gap to maximize separation:
x_domains = [[0.035, 0.265], [0.385, 0.615], [0.735, 0.965]]
y_domain = [0.07, 0.92]

# Horizontal positions for the colorbars, shifted to sit inside the wider gaps
cbar_x_positions = [
    x_domains[0][1] + 0.012,
    x_domains[1][1] + 0.012,
    x_domains[2][1] + 0.012,
...
for i, (name, m) in enumerate(metrics.items()):
    ...
    # ── Identify and highlight the optimal 2D coordinate ──
    if m["better"] == "↑":
        flat_idx = np.nanargmax(Z)
    else:
        flat_idx = np.nanargmin(Z)

    # Translate flat index into 2D row/column grid indices
    row_idx, col_idx = np.unravel_index(flat_idx, Z.shape)
    opt_K = K_vals[col_idx]
    opt_fp = fp_vals[row_idx]

    # Horizontal dashed indicator line highlighting the optimal fp level
    fig.add_trace(
        go.Scatter(
            x=[K_vals[0] - 1.5, K_vals[-1] + 1.5],
            y=[opt_fp, opt_fp],
            mode="lines",
            line=dict(color="#d62728", width=0.7, dash="dash"),
            showlegend=False,
            hoverinfo="skip",
        ),
        row=1,
    ...
    # Distinctive star marker positioned at the exact optimal (K, fp) grid intersection
    fig.add_trace(
        go.Scatter(
            x=[opt_K],
            y=[opt_fp],
            ...
            name=f"Optimum: K={opt_K}, fp={opt_fp}",
            showlegend=False,
        ),
        row=1,
        col=col,
...
# Apply explicit shrunken domains to subplots
fig.update_layout(
...
# Calculate the exact center point for each domain array to properly align the text labels
x_centers = [sum(domain) / 2 for domain in x_domains]

# Style and precisely position the subplot titles cleanly above the shrunken frames
...
out_html = "./drbokeh_surface_plots_stage_1.html"
fig.write_html(out_html)
print(f"Saved Interactive Plot → {out_html}")

out = "./drbokeh_surface_plots_stage_1.png"
try:
    fig.write_image(out, width=WIDTH_PX, height=HEIGHT_PX, scale=SCALE)
    print(f"Saved → {out}")
except Exception:
    print(
        "Static image generation requires 'pip install kaleido'. Open the HTML output to view."
    )
...
# ── Load & parse ──────────────────────────────────────────────────────────────
col_names = ["folder_name", "col1", "col2", "col3", "col4", "col5", "PSNR", "SSIM", "LPIPS"]
raw = pd.read_csv("./Drbokeh/results_drbokeh_fl28.csv", names=col_names, header=None, low_memory=False)
...
metrics = {
    ...
    "LPIPS": {"grid": make_grid("LPIPS"), "colorscale": "turbo", "reversescale": False, "label": "LPIPS",     "better": "↓", "find_opt": "min"},
}
