# -*- coding: utf-8 -*-
"""tempcopy_of_Eval_Metrics_DoF_Latest.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/15D-5NFOSMCnSxDJwn_8QDw1hq6TyiQrT

**Example:**
```
evaluate_method(
    root_dir          = "/path/to/inference_rootdir",
    method_name       = "DrBokeh",
    subfolder_pattern = "fl_*",           # subfolder name patterns, e.g. fl_70, fl_60, ...
    pred_subdir       = "Drbokeh_K25_fp0p25_aligned",  # within each matched subfolder
    gt_subdir         = "F2.8_align",
    output_csv        = "results_drbokeh.csv",
    downscale_factor  = 2,                # matches original 2x downscale
    show_debug        = False,
)
```
"""

!pip install lpips scikit-image torch torchvision pillow opencv-python -q
import os
import csv
import fnmatch
...
def _write_csv(output_csv, root_dir, method_name, subfolder_pattern,
               pred_subdir, gt_subdir, results):
    """
    Writes metric results to a CSV.

    CSV layout:
      - Info rows (root_dir, method_name, subfolder_pattern, pred_subdir, gt_subdir)
      - Blank row
      - K rows for PSNR  : folder_name, [5 empty cols], value
      - Blank row
      - K rows for SSIM  : folder_name, [5 empty cols], value
      - Blank row
      - K rows for LPIPS : folder_name, [5 empty cols], value
    """
    ...
    with open(output_csv, "w", newline="") as f:
        ...
        writer.writerow(["gt_subdir",          gt_subdir])
        writer.writerow([])

        # --- PSNR block ---
        for folder, psnr, _ssim, _lp in results:
            val = f"{psnr:.4f}" if psnr is not None else "N/A"
            writer.writerow([folder] + EMPTY + [val])
        writer.writerow([])

        # --- SSIM block ---
        for folder, _psnr, ssim, _lp in results:
            val = f"{ssim:.4f}" if ssim is not None else "N/A"
            writer.writerow([folder] + EMPTY + [val])
        writer.writerow([])

        # --- LPIPS block ---
        for folder, _psnr, _ssim, lp in results:
            val = f"{lp:.4f}" if lp is not None else "N/A"
            writer.writerow([folder] + EMPTY + [val])
...
def evaluate_method(
    ...
    in <subfolder>/<pred_subdir> and <subfolder>/<gt_subdir>. Results are printed and
    saved to output_csv.
    ...
    Args:
        ...
        output_csv:         Path where the output CSV is written.
...
# for fl in fls:
#     rdir = f"/content/drive/MyDrive/DOF_benchmarking/inference/fl_{fl}"

#     evaluate_method(
#             root_dir          = rdir+"/bokehme_tuning_p1",
#             method_name       = mname,
#             subfolder_pattern = f"{mname}_dfs??_K??_dispfocus?.??", # bokehme_dfs30_K60_dispfocus0.55
#             pred_subdir       = "",
#             gt_path         = f"{rdir}/F2.8_align_whitebal",
#             output_csv        = f"results_{mname}.csv",
#             downscale_factor  = 2,
#             show_debug        = False,
#         )

# Example: Bokehlicious
mname = "bokehlicious"
fls = [36,45,60,70] # [28,]
...
for fl in fls:
    ...
    evaluate_method(
            root_dir          = rdir+"/bokehlicious_p2",
            method_name       = mname,
            subfolder_pattern = f"{mname}_F?.?_intp",
            pred_subdir       = "",
            gt_path         = f"{rdir}/F2.8_align_whitebal",
            output_csv        = f"results_{mname}_p2.csv",
            downscale_factor  = 2,
            show_debug        = False,
        )
...
for fl in fls:
    ...
    evaluate_method(
            root_dir          = rdir,
            method_name       = mname,
            subfolder_pattern = f"{mname}_K??_fp_?.?",
            pred_subdir       = "demo",
            gt_path         = f"{rdir}/F2.8_align_whitebal",
            output_csv        = f"results_{mname}.csv",
            downscale_factor  = 2,
...
for fl in fls:
    ...
    evaluate_method(
            root_dir          = rdir+"/bokehme_tuning_p2",
            method_name       = mname,
            subfolder_pattern = f"{mname}_dfs??_K??_dispfocus?.??", # bokehme_dfs30_K60_dispfocus0.55
            pred_subdir       = "",
            gt_path         = f"{rdir}/F2.8_align_whitebal",
